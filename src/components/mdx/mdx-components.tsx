...
const components = {
  ...
  ul: ({ className, ...props }: React.HTMLAttributes<HTMLUListElement>) => (
    <ul className={cn("my-6 list-disc md:ml-6", className)} {...props} />
  ),
  ol: ({ className, ...props }: React.HTMLAttributes<HTMLOListElement>) => (
    <ol className={cn("my-6 list-decimal md:ml-6", className)} {...props} />
  ),
  ...
  pre: ({ className, ...props }: React.HtmlHTMLAttributes<HTMLPreElement>) => (
    <pre
      className={cn(
        "relative mb-4 mt-6 max-h-[640px] overflow-x-auto rounded-lg border bg-muted p-4 font-mono text-sm font-semibold text-muted-foreground",
        className,
      )}
      {...props}
    />
